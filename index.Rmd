...
socv <- st_read("./sv_spatial_places.shp") %>% select(NAME, State, cluster, geometry)
ener <- st_read("./en_spatial_places.shp") %>% select(NAME, State, cluster, geometry)
typ <- st_read("./typology_2.shp") %>% select(NAME, State, ctypo, ecn_cls, en_clst, sv_clst, geometry)
...
require(paletteer)
cp <-paletteer_c("ggthemes::Sunset-Sunrise Diverging", 33)
cp
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
mapview(socv, zcol = "cluster", col.regions=pallete2)
```

```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(socv, aes(x=cluster, fill = cluster)) + 
  geom_bar()+
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
mapview(ener, zcol = "cluster", col.regions= pallete2)
```

```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(ener, aes(x=cluster, fill = cluster)) + 
  geom_bar()+
    scale_fill_manual(values=c("#6e7cb9","#7bbcd5", "#f5db99"))+
  geom_text(stat='count', aes(label=after_stat(count)), vjust=-0.25)+
  theme_bw()+
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
mapview(typ, zcol = "ctypo", col.regions = cp)
```

```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(typ, aes(x=ctypo, fill = ctypo)) + 
  geom_bar()+
